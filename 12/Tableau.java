...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        if(smallest == null){ //Checked
            Cell initial = new Cell(value);
            smallest = initial;
            smallest.above = null;
            smallest.below = null;
            smallest.left = null;
            ...
            Cell working = smallest;
            Integer popped = addToRow(working, value);//..
            
            if(popped == null){//Checked
                return;   
            }else{
                while(popped != null){
                    
                    if(working.below == null){
                        Cell row = new Cell(popped);
                        row.above = working;
                        working.below = row;
                        row.left = null;
                        row.right = null;
                        ...
                        popped = addToRow(working, popped);
                    }
                }
            }
        }
            ...
            if(curr.right == null){
                ...
                if(curr.above != null){
                    if(curr.above.right != null){
                        endrow.above = curr.above.right;
                        curr.above.right.below = endrow;
                    }else{
                        endrow.above = null;
                    }
                }else{
                ...
                if(curr.below != null){
                    if(curr.below.right != null){
                        endrow.below = curr.below.right;
                        curr.below.right.above = endrow;
                    }else{
                        endrow.below = null;
                    }
                }else{
